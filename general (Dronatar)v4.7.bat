...
--comment Telegram (WebRTC) [WIP] ^
--filter-udp=1400 ^
--filter-l7=stun ^
--dpi-desync=fake ^
--dpi-desync-fake-stun=0x00 --new ^

--comment Discord (WebRTC) ^
...
--dpi-desync=fake ^
--dpi-desync-fake-tls-mod=sni=web.vk.me ^
--dpi-desync-fooling=badseq ^
--dpi-desync-badseq-increment=0 ^
--dpi-desync-badack-increment=1 ^
--dpi-desync-repeats=6 --new ^

--comment list-google(YouTube QUIC)/list-general(QUIC) ^
--filter-udp=443 ^
...
--comment list-google(YouTube Streaming)/list-general(HTTP) ^
--filter-tcp=80 ^
--hostlist="%LISTS%list-google.txt" ^
--hostlist="%LISTS%list-general.txt" ^
--dpi-desync=fake ^
...
--dpi-desync-repeats=4 --new ^

--comment IP set(TCP 80,443) ^
--filter-tcp=80,443 ^
